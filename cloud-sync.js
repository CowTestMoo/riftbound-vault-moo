(() => {
  ...
  function renderCloudUI(){ensureAuthDialog();const signed=!!session?.user,account=document.getElementById('cloudAccountBtn'),text=document.getElementById('cloudSettingText');if(account)account.textContent=signed?'Account':'Sign In';if(text)text.textContent=signed?`Signed in • Event-driven sync • ${document.getElementById('cloudSyncStatus')?.textContent||'Ready'}`:'Invite-only • Local only'}
