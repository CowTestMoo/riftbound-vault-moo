(() => {
  ...
  async function signUp(email,password){
    ...
    if(data.access_token){saveSession(normalizeSession(data));await reconcileAfterLogin();return {signedIn:true,message:'Account created and signed in.'}}
    return {signedIn:false,message:'Account created. Check your email to confirm it, then sign in.'};
  }
  ...
  async function reconcileAfterLogin(){
    ...
    if(!remote){await pushRemote(true);return}
    ...
    if(meaningful(local)&&!meaningful(remoteState)){await pushRemote(true);return}
    if(m.lastRemoteAt===remote.updated_at&&m.lastSyncedHash&&m.lastSyncedHash!==localHash){await pushRemote(true);return}
  ...
  function ensureAuthDialog(){
    ...
    d.innerHTML=`<div class="modal-inner cloud-auth-inner"><div class="modal-head"><h2>Riftbound Cloud</h2><button class="close-btn" data-cloud-close>×</button></div><p class="cloud-help">Sign in to sync this vault between devices.</p><label>Email<input id="cloudEmail" type="email" autocomplete="email" placeholder="you@example.com"></label><label>Password<input id="cloudPassword" type="password" autocomplete="current-password" minlength="6" placeholder="Password"></label><div id="cloudAuthMessage" class="cloud-auth-message"></div><div class="modal-actions"><button id="cloudSignIn" class="primary-btn" type="button">Sign In</button><button id="cloudSignUp" class="ghost-btn" type="button">Create Account</button></div></div>`;
  ...
  function ensureCloudSettings(){
    ...
    row.innerHTML=`<div class="setting-copy"><strong>Cloud sync</strong><small id="cloudSettingText">Local only</small></div><div class="cloud-setting-actions"><button id="cloudAccountBtn" class="sound-test" type="button">Sign In</button><button id="cloudSyncBtn" class="sound-test" type="button" hidden>Sync</button></div>`;
  ...
  function renderCloudUI(){
    ...
    if(text)text.textContent=signed?`${session.user.email||'Signed in'} • ${document.getElementById('cloudSyncStatus')?.textContent||'Ready'}`:'Local only';
  }
  function ensureStatus(){
    if(document.getElementById('cloudSyncStatus'))return;
    const p=document.getElementById('catalogStatus');if(!p)return;
    const s=document.createElement('span');s.id='cloudSyncStatus';s.className='cloud-sync-status';s.textContent=session?'Cloud ready':'Local only';p.insertAdjacentElement('afterend',s);
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    if(e.target.closest('#cloudSyncBtn')){await pushRemote(true);return}
    ...
    if(e.target.closest('#cloudSignUp')){
      const email=document.getElementById('cloudEmail').value.trim(),password=document.getElementById('cloudPassword').value,msg=document.getElementById('cloudAuthMessage');
      msg.textContent='Creating account…';try{const r=await signUp(email,password);msg.textContent=r.message;if(r.signedIn)document.getElementById('cloudAuthDialog').close()}catch(err){msg.textContent=err.message}return
    }
  ...
  async function init(){
    ensureStatus();ensureCloudSettings();setStatus(session?'Cloud ready':'Local only',session?'idle':'idle');
    if(session){try{await freshSession();if(session)await reconcileAfterLogin()}catch(err){console.error(err);setStatus('Sync error','error')}}
  ...
  window.RiftboundCloud={syncNow:()=>pushRemote(true),signIn,signUp,signOut,getSession:()=>session,reconcile:reconcileAfterLogin};
