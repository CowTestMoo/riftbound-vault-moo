(() => {
  ...
  async function consumeInviteHash(){
    ...
    try{const user=await request('/auth/v1/user',{token:access});saveSession({access_token:access,refresh_token:refresh,expiresAt:Date.now()+Number(p.get('expires_in')||3600)*1000,user});history.replaceState(null,'',location.pathname+location.search);if(type==='invite'||type==='recovery'){showPasswordSetup(type);return true}await reconcileAfterLogin();return true}catch(err){console.error('Invite link could not be accepted',err);return false}
  }
