(() => {
  ...
  function returnToOwnVault(){
    ...
    setTimeout(()=>{
      ...
      document.getElementById('uxSettings')?.setAttribute('hidden','');
    },390);
  ...
  /* Register before the social/audio handlers so returning home is one clean transition. */
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('#friendBrowseAnother'))setTimeout(polishProfileControls,0);
  });
