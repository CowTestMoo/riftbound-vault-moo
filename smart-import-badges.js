(() => {
  ...
  function observeDialog(){
    const dialog=document.getElementById('spreadsheetImportDialog');
    if(!dialog)return false;
    observer.disconnect();observer.observe(dialog,{childList:true,subtree:true});return true;
  }
  function init(){
    if(!observeDialog()){
      const boot=new MutationObserver(()=>{if(observeDialog())boot.disconnect()});
