(() => {
  ...
  async function api(path,{method='GET',body,token,prefer}={}){
    const res=await fetch(SUPABASE_URL+path,{method,headers:{apikey:SUPABASE_KEY,'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{Authorization:`Bearer ${SUPABASE_KEY}`}),...(prefer?{Prefer:prefer}:{})},body:body===undefined?undefined:JSON.stringify(body)});
  ...
  function ensureTopbar(){
    ...
    const s=session();
    area.innerHTML=`<button id="browseLibrariesBtn" class="ghost-btn social-browse-btn" type="button">Browse Libraries</button>${s?.user?`<button id="usernameChip" class="username-chip" type="button" title="Your public username">${profile?`@${esc(profile.username)}`:'Set username'}</button>`:''}`;
  }
  ...
  function ensureDialogs(){
    if(!document.getElementById('usernameSetupDialog')){
      ...
      d.innerHTML=`<div class="modal-inner username-setup-inner"><div class="username-mark">@</div><h2>Choose your username</h2><p>Your username is required so friends can find your public card library. It does not change how you sign in.</p><label>Username<div class="username-input-wrap"><span>@</span><input id="usernameInput" maxlength="24" autocomplete="off" placeholder="riftbound_friend"></div></label><small class="username-rules">3–24 characters. Letters, numbers, and underscores only.</small><div id="usernameMessage" class="feature-message"></div><button id="saveUsernameBtn" class="primary-btn" type="button">Claim Username</button></div>`;
  ...
  async function loadOwnProfile(){
    const s=session();if(!s?.access_token||!s?.user)return null;
    ...
    profile=Array.isArray(rows)&&rows[0]?rows[0]:null;ensureTopbar();
    if(!profile)showUsernameSetup();else schedulePublish(20);
    return profile;
  }

  function showUsernameSetup(){
    ensureDialogs();const d=document.getElementById('usernameSetupDialog');if(!d.open)d.showModal();setTimeout(()=>document.getElementById('usernameInput')?.focus(),40);
  }
  ...
  async function saveUsername(){
    ...
    try{
      ...
      msg.textContent='Username saved.';ensureTopbar();schedulePublish(20);setTimeout(()=>document.getElementById('usernameSetupDialog')?.close(),350);
  ...
  function publicCards(){
    const inv=readState().inventory||{},out={};
    for(const [code,row] of Object.entries(inv)){const n=Math.max(0,Math.floor(Number(row?.owned||0)));if(n>0)out[code]=n}
    return out;
  }
  async function publishLibrary(){
    const s=session();if(!s?.access_token||!s?.user||!profile||publishing)return;
    publishing=true;
    try{await api('/rest/v1/public_libraries?on_conflict=user_id',{method:'POST',token:s.access_token,prefer:'resolution=merge-duplicates,return=minimal',body:{user_id:s.user.id,cards:publicCards(),updated_at:new Date().toISOString()}})}
    catch(err){console.error('Public library publish failed',err)}finally{publishing=false}
  }
  ...
  async function loadPublicData(){
    const [profiles,libraries]=await Promise.all([
      api('/rest/v1/profiles?select=user_id,username,updated_at&order=username.asc'),
      api('/rest/v1/public_libraries?select=user_id,cards,updated_at')
    ]);
    browseProfiles=Array.isArray(profiles)?profiles:[];
    browseLibraries=new Map((Array.isArray(libraries)?libraries:[]).map(x=>[x.user_id,x]));
  }

  function renderProfileList(q=''){
    const root=document.getElementById('publicProfileList');if(!root)return;
    const needle=String(q||'').trim().toLowerCase();
    const list=browseProfiles.filter(p=>!needle||p.username.toLowerCase().includes(needle));
  ...
  async function openBrowser(){
    ensureDialogs();const d=document.getElementById('publicLibrariesDialog');d.showModal();
    document.getElementById('publicProfileList').innerHTML='<div class="recent-empty">Loading libraries…</div>';
    try{await loadPublicData();renderProfileList();if(browseProfiles.length&&!browseUserId){browseUserId=browseProfiles[0].user_id}renderProfileList();renderPublicLibrary()}catch(err){document.getElementById('publicProfileList').innerHTML=`<div class="recent-empty">Could not load libraries: ${esc(err.message)}</div>`}
  }

  function bind(){
    document.addEventListener('click',e=>{
      if(e.target.closest('#browseLibrariesBtn,#vaultLockBrowse')){openBrowser();return}
      if(e.target.closest('[data-close-public-libraries]')){document.getElementById('publicLibrariesDialog')?.close();return}
      if(e.target.closest('#usernameChip')){showUsernameSetup();if(profile){const input=document.getElementById('usernameInput');input.value=profile.username;document.getElementById('usernameMessage').textContent='Changing your username changes how friends find you.'}return}
      if(e.target.closest('#saveUsernameBtn')){saveUsername();return}
      const user=e.target.closest('[data-public-user]');if(user){browseUserId=user.dataset.publicUser;browseQuery='';const search=document.getElementById('publicCardSearch');if(search)search.value='';renderProfileList(document.getElementById('publicProfileSearch')?.value||'');renderPublicLibrary();return}
    },true);
    document.addEventListener('input',e=>{
      if(e.target.id==='publicProfileSearch')renderProfileList(e.target.value);
      if(e.target.id==='publicCardSearch'){browseQuery=e.target.value;renderPublicLibrary()}
    });
    ...
    window.addEventListener('riftbound-auth-storage-change',()=>setTimeout(()=>{ensureTopbar();loadOwnProfile().catch(console.error)},50));
  }

  async function init(){ensureDialogs();ensureTopbar();bind();if(session()?.user){try{await loadOwnProfile()}catch(err){console.error('Profile load failed',err)}}const observer=new MutationObserver(()=>ensureTopbar());observer.observe(document.body,{childList:true,subtree:true})}
