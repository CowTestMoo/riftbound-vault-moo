(() => {
  ...
  function apply(){
    ...
    if(title)title.textContent=isNeon?'Neon audio':'Cosmic audio';if(help)help.textContent=isNeon?'Cyberpunk data chirps, synth pulses, glitches, and terminal clicks.':'Spatial chimes, crystalline taps, and deep-space interface tones.';if(volTitle)volTitle.textContent=isNeon?'Neon volume':'Cosmic volume';if(toggle)toggle.checked=enabled(t,s);const v=Math.round(volume(t,s)*100);if(range)range.value=String(v);if(level)level.textContent=`${v}%`
  }

  function switchTheme(value){if(value==='neon')writeUX({intensity:'neon',cosmicSound:false,sound:false});else writeUX({intensity:'supernova',neonSound:false,sound:false});apply();play('switch',true)}
  ...
  function neon(kind,v){const c=audioContext(),t=c.currentTime,base=kind==='transition'?98:kind==='remove'?120:kind==='add'?220:kind==='switch'?165:kind==='hover'?360:280,seq=kind==='transition'?[1,2.03,.75,3.04,1.5]:kind==='complete'?[1,1.5,2,3]:kind==='switch'?[1,2.04,1.5]:kind==='remove'?[1,.68]:[1,2.02];seq.forEach((m,i)=>tone(c,base*m,i%2?'square':'sawtooth',t+i*(kind==='transition'?.038:.022),kind==='transition'?.13:.075+i*.012,v*((kind==='transition'?.08:.06)/(1+i*.3)),kind==='remove'?.78:1.08));if(kind!=='hover')noise(c,t+.004,kind==='transition'?.12:.035,v*(kind==='transition'?.035:.022),kind==='transition'?1100:1500)}
  ...
  document.addEventListener('change',e=>{if(e.target.id==='intensitySelect'){switchTheme(e.target.value);return}if(e.target.id==='themeAudioToggle'){const t=theme(),on=e.target.checked;writeUX(t==='neon'?{neonSound:on,cosmicSound:false,sound:false}:{cosmicSound:on,neonSound:false,sound:false});apply();if(on)play('switch')}} ,true);
  ...
  document.addEventListener('click',e=>{if(e.target.closest('#themeAudioTest')){const t=theme();writeUX(t==='neon'?{neonSound:true,cosmicSound:false}:{cosmicSound:true,neonSound:false});apply();play('complete');return}if(e.target.closest('[data-adjust],[data-bulk],[data-fast-bulk]')){const b=e.target.closest('[data-adjust],[data-bulk],[data-fast-bulk]'),n=Number(b?.dataset.adjust??b?.dataset.bulk??b?.dataset.delta??0);play(n<0?'remove':'add');return}if(e.target.closest('.tab,[data-mobile-tab],.filter-chip,.storage-box,button'))play('click');if(e.target.closest('[data-tab="storage"],#customizeStorageBtn,#saveStorageBoxes'))setTimeout(decorateStorage,60)},true);
  ...
  window.addEventListener('riftbound-cloud-restored',()=>setTimeout(refresh,50));window.addEventListener('riftbound-local-change',e=>{if(e.detail?.key===APP_KEY)setTimeout(decorateStorage,70)});window.addEventListener('riftbound-social-ready',()=>setTimeout(refresh,20));
