(() => {
  ...
  function ensureButton(){
    ...
    const header=screen.querySelector('.friend-header'),browse=document.getElementById('friendBrowseAnother');if(!header)return;
    let actions=header.querySelector('.friend-header-actions');
    if(!actions){actions=document.createElement('div');actions.className='friend-header-actions';header.appendChild(actions);if(browse)actions.appendChild(browse)}
